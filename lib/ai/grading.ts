...
function mockGrade(
  ...
  const mathSamples = [
    { text: "16 × 2 =", right: "32", wrongs: ["28", "30"], analysis: "个位相乘后进位处理错误，6×2=12 应向十位进 1。" },
    { text: "73 − 12 =", right: "61", wrongs: ["51", "65"], analysis: "不需要退位却误退位，十位计算出错。" },
    { text: "52 ÷ 7 =", right: "7余3", wrongs: ["7余2", "6余10"], analysis: "余数计算错误，52−49=3。" },
    { text: "38 + 25 =", right: "63", wrongs: ["53", "62"], analysis: "进位加法漏加进位 1。" },
    { text: "1米 = ( )厘米", right: "100", wrongs: ["10", "1000"], analysis: "长度单位换算进率记忆不牢。" },
    { text: "小明有12支笔，是小红的3倍，小红有几支？", right: "4", wrongs: ["36", "9"], analysis: "倍数关系理解反了，应为 12÷3。" },
  ];
  const chnSamples = [
    { text: "「认识」的「识」", right: "识", wrongs: ["帜", "织"], analysis: "形近字混淆，注意言字旁与巾字旁的区别。" },
    { text: "一（ ）鱼", right: "条", wrongs: ["只", "个"], analysis: "量词搭配不当，鱼用「条」。" },
    { text: "「长」在「长大」中读", right: "zhǎng", wrongs: ["cháng"], analysis: "多音字辨析错误，表示生长时读 zhǎng。" },
    { text: "照样子写句子：花儿开了。", right: "鸟儿叫了。", wrongs: ["鸟儿在树上。"], analysis: "句式仿写未保持「主语+动词+了」结构。" },
    { text: "找出短文的中心句", right: "第一句", wrongs: ["最后一句"], analysis: "阅读理解中未抓住段落关键句。" },
  ];
  ...
  for (let i = 0; i < nQ; i++) {
    ...
    const kp = matchKnowledgePoint(undefined, subject, kps) ?? pool[(seed + i) % pool.length];
    results.push({
      ...
      confidence: 0.82 + ((seed + i * 3) % 15) / 100,
    });
  }
